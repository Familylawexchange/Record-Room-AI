...
function renderReview() {
  app.innerHTML = `<section class="card panel reviewPage"><p class="eyebrow">Admin review workflow</p><h2 class="pageTitle">Review Queue</h2><p class="formIntro">Review pending uploads before publication. Use filters to quickly locate records by status, person, court, or case details.</p><div class="reviewTabs" id="review-status-tabs"><button type="button" class="reviewTab active" data-review-status="">All</button><button type="button" class="reviewTab" data-review-status="pending">Pending</button><button type="button" class="reviewTab" data-review-status="approved">Approved</button><button type="button" class="reviewTab" data-review-status="rejected">Rejected</button></div><form class="reviewFilters" id="admin-filters"><label>Status${selectHtml('status', [''].concat(reviewStatuses), '')}</label><label>Keyword<input name="q" placeholder="name, court, case, text" /></label><label>Sort by<select name="sort"><option value="newest" selected>Newest first</option><option value="oldest">Oldest first</option></select></label><button type="submit">Apply filters</button><button type="button" id="admin-filters-reset" class="buttonSecondary">Reset</button></form><div id="admin-list-meta" class="reviewMeta empty">Loading records...</div><div class="actionsList reviewActions">${['approve public','approve private','reject','mark duplicate','mark needs redaction','mark needs official verification','attach to profile','create profile','create claim','edit metadata'].map(pill).join('')}</div><div id="admin-list" class="adminList reviewList"></div></section>`;
}
...
function wireRoute() {
  ...
  document.querySelector('#admin-filters-reset')?.addEventListener('click', () => {
    const form = document.querySelector('#admin-filters');
    if (!form) return;
    form.reset();
    const statusSelect = form.querySelector('[name="status"]');
    if (statusSelect) statusSelect.value = '';
    document.querySelectorAll('.reviewTab').forEach((btn) => btn.classList.toggle('active', btn.dataset.reviewStatus === ''));
    loadAdminUploads();
  });
  document.querySelectorAll('.reviewTab').forEach((btn) => btn.addEventListener('click', () => {
    const form = document.querySelector('#admin-filters');
    if (!form) return;
    const statusSelect = form.querySelector('[name="status"]');
    if (statusSelect) statusSelect.value = btn.dataset.reviewStatus || '';
    document.querySelectorAll('.reviewTab').forEach((tab) => tab.classList.toggle('active', tab === btn));
    loadAdminUploads();
  }));
...
function renderDocumentsFlowRow(doc) { let analysis = {}; try { analysis = JSON.parse(doc.ai_summary_json || '{}'); } catch {} const actors = [ ...(analysis.judges || []), ...(analysis.attorneys || []), ...(analysis.guardiansAdLitem || []) ].slice(0, 4).join(', '); return `<tr><td>${escapeHtml(doc.original_filename || '')}<div class="statusDetail">${escapeHtml([doc.document_type, doc.court, doc.county || doc.state].filter(Boolean).join(' · '))}</div></td><td>${escapeHtml(new Date(doc.created_at).toLocaleString())}</td><td>${escapeHtml(formatBytes(doc.file_size || 0))}</td><td>${pill(doc.uploadStatusLabel || 'Uploaded')}</td><td>${pill(doc.aiIndexingStatusLabel || 'Processing')} ${analysis.recommendedHumanReview ? pill('recommended human review') : ''}<div class="statusDetail">${escapeHtml((analysis.patternTags || []).slice(0,3).join(', ') || doc.flowMessage || '')}</div><div class="statusDetail">${escapeHtml(actors ? `Key actors: ${actors}` : '')}</div><div class="statusDetail">${escapeHtml((analysis.extractionWarnings || []).slice(0,2).join(' | '))}</div></td><td><button type="button" data-delete-document="${doc.id}">Delete</button></td></tr>`; }
...
async function loadAdminUploads(showErrors = true) { const el = document.querySelector('#admin-list'); const meta = document.querySelector('#admin-list-meta'); const form = document.querySelector('#admin-filters'); if (!el || !form) return; el.innerHTML = '<p class="empty">Loading records...</p>'; if (meta) meta.textContent = 'Loading records...'; try { const formData = new FormData(form); const sortBy = String(formData.get('sort') || 'newest'); const q = new URLSearchParams(formData); q.delete('sort'); const payload = await api(`/api/admin/uploads?${q}`, { headers: adminHeaders() }); const docs = [...(payload.documents || [])].sort((a, b) => { const da = new Date(a?.created_at || 0).getTime(); const db = new Date(b?.created_at || 0).getTime(); return sortBy === 'oldest' ? da - db : db - da; }); if (meta) meta.textContent = docs.length ? `${docs.length} record(s) found` : 'No records matched your filters.'; el.innerHTML = docs.length ? docs.map(renderAdminCard).join('') : '<p class="empty">No records matched.</p>'; el.querySelectorAll('[data-save-upload]').forEach((b) => b.addEventListener('click', saveUploadEdits)); el.querySelectorAll('[data-show-text]').forEach((b) => b.addEventListener('click', showExtractedText)); el.querySelectorAll('[data-create-profile]').forEach((b) => b.addEventListener('click', createProfileFromUpload)); } catch (e) { if (meta) meta.textContent = 'Failed to load review queue.'; if (showErrors) el.innerHTML = `<p class="status error">${escapeHtml(e.message)}</p>`; } }
...
async function submitLead(event) { event.preventDefault(); const status = document.querySelector('#lead-status'); try { const res = await fetch(apiUrl('/api/admin/research-leads'), { method: 'POST', headers: adminHeaders(), body: new FormData(event.currentTarget) }); const payload = await res.json().catch(() => ({})); if (!res.ok) throw new Error(requestErrorMessage(apiUrl('/api/admin/research-leads'), res.status, payload)); setStatus(status, 'Research lead saved for review.', 'success'); event.currentTarget.reset(); loadLeads(); } catch (e) { setStatus(status, e.message, 'error'); } }
...
function renderAdminCard(doc) { return `<article class="adminCard reviewCard" data-upload-id="${doc.id}"><div class="adminCardHeader"><div><h3>#${doc.id} ${escapeHtml(doc.document_title || doc.original_filename)}</h3><p class="reviewCardPills">${pill(doc.review_status)} ${pill(doc.visibility)} ${pill(doc.redaction_status)} ${pill(doc.extraction_status)}</p></div><div class="adminActions"><a class="buttonLink" href="${apiUrl(`/api/admin/uploads/${doc.id}/download`)}?token=${encodeURIComponent(adminToken())}" target="_blank" rel="noopener">Download original</a><button type="button" data-show-text="${doc.id}">View extracted text</button><button type="button" data-create-profile="${doc.id}">Create profile</button></div></div><div class="reviewCardFacts">${['original_filename','document_type','source_type','source_label','state','county','court','case_number','subject_name','subject_role','created_at'].map((k) => `<div class="reviewFact"><span>${k.replaceAll('_',' ')}</span><strong>${escapeHtml(doc[k] || '-')}</strong></div>`).join('')}</div><details class="reviewEditDetails"><summary>Edit metadata and review status</summary><div class="form-grid grid three editGrid">${editInput('subject_name', doc.subject_name)}${editInput('document_type', doc.document_type)}${editInput('source_type', doc.source_type)}${editInput('state', doc.state)}${editInput('county', doc.county)}${editInput('court', doc.court)}${editInput('case_number', doc.case_number)}${editSelect('review_status', doc.review_status, ['pending','approved','approved public','approved private','rejected','needs redaction','needs official source verification','duplicate','sealed/do not publish'])}${editSelect('visibility', doc.visibility, ['private','public','admin-only'])}${editSelect('redaction_status', doc.redaction_status, ['not_requested','needs redaction','redacted','sealed/do not publish'])}</div><label>Reliability tags<input data-field="reliability_tags" value="${escapeHtml(doc.reliability_tags || '')}" /></label><label>Tags/allegation categories<input data-field="tags" value="${escapeHtml(doc.tags || '')}" /></label><label>Public summary<textarea data-field="public_summary">${escapeHtml(doc.public_summary || '')}</textarea></label><details><summary>AI-ready source-bound summary structure</summary><pre>${escapeHtml(doc.ai_summary_json || '{}')}</pre></details><button type="button" data-save-upload="${doc.id}">Save metadata/status</button></details><div class="extractedText" id="text-${doc.id}">${escapeHtml(doc.extraction_preview || '[No extracted preview available.]')}</div></article>`; }
...
function requestErrorMessage(endpoint, status, payload = {}) {
  const base = payload?.error || payload?.message || `Request failed with ${status}`;
...
async function api(url, options = {}) {
  ...
  try {
    ...
    throw new Error(`Network error while calling ${endpoint}: ${error?.message || 'Unknown network error.'}`);
  }
  const payload = await res.json().catch(() => ({}));
  if (!res.ok) throw new Error(requestErrorMessage(endpoint, res.status, payload));
  return payload;
}

...
function renderHome() {
  app.innerHTML = `<section class="hero card panel"><div><p class="badge">Source-labeled legal accountability database</p><h1>Record Room AI</h1><p class="lede">Private intake, public submissions, admin review, source-bound records, research leads, connector-safe scanner placeholders, and public search limited to approved public records.</p><div class="guardrails"><span>Private by default</span><span>No auto-publication</span><span>SQLite + local files</span><span>Source-bound AI-ready summaries</span></div></div></section>${flowPanel()}`;
}
...
function flowPanel() { return `<section class="card panel"><p class="eyebrow">Data flow</p><h2>Scanner/API/manual import → review → source-bound public records</h2><p>Scanner/API/manual import → raw_results or research_leads → review_queue → documents/profiles/claims → search_index → AI source-bound summaries → public search only if approved.</p><div class="actionsList">${['No uncontrolled scraping','No sealed/confidential publication','Redaction workflow','Deduplication by hashes/URLs/case metadata','Relevance confidence ≠ truth'].map(pill).join('')}</div></section>`; }
...
function fileInput(required = true) { return `<input name="file" ${required ? 'required' : ''} type="file" accept=".pdf,.docx,.doc,.txt,.jpg,.jpeg,.png,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,text/plain,image/jpeg,image/png" />`; }
...
function validateFile(file) { if (!file) return 'Choose a document file before submitting.'; const ext = file.name.split('.').pop().toLowerCase(); if (!allowedExtensions.includes(ext)) return 'Unsupported file type. Accepted: PDF, DOCX, DOC, TXT, JPG, JPEG, PNG.'; if (file.size > MAX_FILE_BYTES) return 'File is too large. Maximum size is 25 MB per file.'; return ''; }
async function api(url, options = {}) { const headers = new Headers(options.headers || {}); if (url.startsWith('/api/admin/') && !url.startsWith('/api/admin/login') && adminToken()) headers.set('X-Admin-Token', adminToken()); const res = await fetch(url, { ...options, headers }); const payload = await res.json().catch(() => ({})); if (!res.ok) throw new Error(payload.error || `Request failed with ${res.status}`); return payload; }
